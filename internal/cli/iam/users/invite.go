...
func InviteBuilder() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`A MongoDB user account grants access only to the the MongoDB application. To grant database access, create a database user with %s dbusers create.
		
User accounts and API keys with any role can run this command.`, cli.ExampleAtlasEntryPoint()),
		Args: require.NoArgs,
